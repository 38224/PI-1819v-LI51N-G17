...
describe('LastFm - data Tests', () => {
    ...
    it('should get all artist with ffd', done => {
        lastFMData.getArtistsByName('Fat Freddy s Drop', (err, data) => {
            if (!err) {
                expect(data)
                    .to.be.an('array')
    ...
    it('should get a albums of an artist', done => {
        lastFMData.getAlbumsByMbid("d451395a-f768-432e-bb70-d38c32baf4cb", (err, data) => {
            if (!err) {
                expect(data)
                    .to.be.an('array')
    ...
    it('should get tracks from an album', done => {
        lastFMData.getTracksByMbid("0b3d401e-aa43-3e84-9b9b-51e0b67bce8a", (err, data) => {
            if (!err) {
                expect(data)
                    .to.be.an('object')
